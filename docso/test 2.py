...
def index():
    ...
    for xe in data:
        ...
        if anh_bien_so:
            anh_bien_so = f"/images/{anh_bien_so}"  # Đường dẫn ảnh trên web
        else:
            anh_bien_so = None
...
def upload_plate():
    ...
    filename = f"{bien_so}_{request.form.get('timestamp', '')}.jpg"
    ...
    return jsonify({"message": "Upload successful", "image_path": f"/images/{filename}"})
